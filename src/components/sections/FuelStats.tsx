...
export default function FuelStats() {
  ...
  const { fuelCostThisYear } = useMaintenanceInfo();
  ...
  const stats = [
    {
      label:
        language === "esp"
          ? "Total gastado en combustible este año:"
          : "Total spent this year on fuel:",
      value: `$${fuelCostThisYear}`,
    },
    {
      label:
        language === "esp"
    ...
    <section className="rounded-lg shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-center gap-6">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="flex items-center gap-2 text-xs shadow-sm 825p:flex-col 825p:gap-0"
          >
